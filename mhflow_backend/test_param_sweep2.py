#!/usr/bin/env python3
"""Test effect of varying HP3 ttd on system performance (after fixes)."""
import sys
...
def run_with_hp3_ttd(ttd_val):
    ...
    return {
        ...
        'iter': result.get('outer_iteration_count', 0),
    }

print("Testing HP3 ttd sweep (after fixes)...")
print(f"{'ttd':>6} {'P_gross':>10} {'P_net':>10} {'HR':>10} {'eta':>8} {'HP3_stm':>10} {'RH_flow':>10} {'iter':>5}")
for ttd in [3.9, 10, 20, 30, 40, 50, 80, 100]:
    r = run_with_hp3_ttd(ttd)
    print(f"{r['ttd']:>6.1f} {r['power_gross']:>10.2f} {r['power_net']:>10.2f} {r['heat_rate']:>10.1f} {r['efficiency']:>8.4f} {r['hp3_steam']:>10.2f} {r['reheat_flow']:>10.2f} {r['iter']:>5}")
