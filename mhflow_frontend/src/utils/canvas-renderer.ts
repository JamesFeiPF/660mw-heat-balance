...
export function drawConnection(
  ...
  // 判断连接类型：能量连接或介质连接
  const isPowerConnection = connection.type === 'power'
  ...
  if (isPowerConnection) {
    // 能量连接：黄色虚线
    lineColor = '#f1c40f'
    lineWidth = 2.5 * transform.scale
    ctx.setLineDash([8 * transform.scale, 4 * transform.scale])
  } else if (solveResult?.success && fromPort) {
    // 介质连接：根据温度着色
  ...
  if (transform.scale > 0.5) {
    ...
    if (isPowerConnection) {
      // 能量连接显示功率
      ctx.font = `${Math.round(10 * transform.scale)}px "Microsoft YaHei", sans-serif`
      const powerLabel = 'W →'
      const textWidth = ctx.measureText(powerLabel).width
      ...
      ctx.textAlign = 'center'
      ctx.textBaseline = 'middle'
      ctx.fillText(powerLabel, midX, midY)
    } else if (solveResult?.success && fromPort) {
      // 介质连接显示参数
      const line1 = `${fromPort.m.toFixed(1)} kg/s | ${fromPort.t.toFixed(1)}°C`
      const line2 = `h=${fromPort.h.toFixed(0)} kJ/kg | p=${fromPort.p.toFixed(2)} MPa`
      ...
      const padding = 4 * transform.scale
      const lineHeight = 14 * transform.scale

      ctx.fillStyle = 'rgba(0, 0, 0, 0.7)'
      ctx.beginPath()
      ctx.roundRect(
        midX - maxWidth / 2 - padding,
        midY - lineHeight - padding,
        maxWidth + padding * 2,
      ...
      ctx.textAlign = 'center'
      ctx.textBaseline = 'middle'
      ctx.fillText(line1, midX, midY - 6 * transform.scale)
      ctx.fillText(line2, midX, midY + 10 * transform.scale)
    }
  }
